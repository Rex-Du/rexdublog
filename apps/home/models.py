...
class Series(models.Model):
    name = models.CharField('系列名', max_length=100)
    # count = models.IntegerField('包含文章数', default=0)
...
class Article(models.Model):
    ...
    image = models.ImageField(
        '图片', upload_to='article/%Y/%m', max_length=100, null=True, blank=True)
    ...
    category = models.ForeignKey(
        Category, verbose_name='类别', on_delete=models.DO_NOTHING)
...
class ReadAndPraise(models.Model):
    ...
    # 直接是收藏对象的id，使用时与fav_type字段一起得出结果
    article_id = models.IntegerField('数据id', default=0)
    category = models.IntegerField('收藏类型', choices=(
        (1, '文章'), (2, '分享'), (3, '其它')), default=1)
    add_time = models.DateTimeField('添加时间', default=timezone.now)
...
class Comment(models.Model):
    ...
    article = models.ForeignKey(
        Article, blank=True, null=True, verbose_name='文章', on_delete=models.DO_NOTHING)
    pid = models.ForeignKey('self', blank=True, null=True,
                            verbose_name='父级评论', on_delete=models.DO_NOTHING)
